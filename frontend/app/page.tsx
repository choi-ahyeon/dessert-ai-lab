...
export default function Home() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/fruits/`)
        ...
        .catch(err => {
          console.error('과일 데이터 불러오기 실패:', err)
        })
  }, [])
  ...
  const handlePairing = async () => {
    ...
    try {
      ...
      const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/fruits/pairing`,
          {
            method: 'POST',
            headers: {
  ...
  return (
      <main className="min-h-screen bg-pink-50 flex flex-col items-center justify-center p-8">
        ...
        <div className="bg-white rounded-3xl shadow-lg p-8 w-full max-w-lg border border-pink-100">
          <h2 className="text-xl font-semibold text-pink-400 mb-6 text-center">
            🎀 과일 조합 만들기
          </h2>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="text-center text-pink-300 text-2xl">
              +
            </div>
              ...
              <div className="mt-8 bg-pink-50 rounded-2xl p-6 border border-pink-100">
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <div className="bg-white rounded-2xl p-4 text-center border border-pink-100">
                    ...
                    <p className="text-2xl font-bold text-pink-400">
                      {result.predicted_sugar
                          ? `${result.predicted_sugar}g`
                          : '-'}
                    </p>
                  </div>
                    ...
                    <div className="bg-white rounded-2xl p-4 border border-pink-100 mb-4">
                      ...
                      <div className="flex flex-col gap-2">
                        {[
                          {
                            label: '단맛',
                            val: result.predicted_sweetness,
                          },
                          {
                            label: '산미',
                            val: result.predicted_acidity,
                          },
                          {
                            label: '쓴맛',
                            val: result.predicted_bitterness,
                          },
                          {
                            label: '크리미함',
                            val: result.predicted_creaminess,
                          },
                          {
                            label: '과즙감',
                            val: result.predicted_juiciness,
                          },
                          {
                            label: '풍미',
                            val: result.predicted_richness,
                          },
                          {
                            label: '상큼함',
                            val: result.predicted_freshness,
                          },
                        ].map(item => (
                            <div
                                key={item.label}
                            ...
                            >
                      <span className="text-xs text-pink-300 w-16">
                        {item.label}
                      </span>
                              ...
                              <div className="flex-1 bg-pink-50 rounded-full h-2">
                                <div
                                    ...
                                    style={{
                                      width: `${((item.val || 0) / 10) * 100}%`,
                                    }}
                                />
                              </div>

                              <span className="text-xs text-pink-400 w-6 text-right">
                        {item.val}
                      </span>
                            </div>
                        ))}
